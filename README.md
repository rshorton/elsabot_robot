...
This project is the integration of the elsabot_4wd and elsabot_jeep projects since both are very similar.

### ebot-2wd:

Tested:
* Bring-up and basic Nav

Untested:
* Test with Gazebo
* Test GPS
* Test Nav Keep out
* Test Fixed-path planner

Improvements:
* Add rear obstacle sensors to ebot-2wd (since backup movements now needed for Ackermann steering support)

### jeep:
* uC Firmware updates needed (merge jeep-specific branch with ackerman branch)
* Full testing   
...
Use the Elsabot Docker (https://github.com/rshorton/elsabot_docker) to setup the ROS2 environment with this package and its dependencies.  If you are using this package on a different robot base, then revise the 'device_env_vars.env' file to specify the various devices used.  Also verify the 'run_ros_docker.sh' script maps your devices.

#### On Development Machine
* Create a ROS2 environment using the Elsabot Docker (robot-specific packages are optional, but you may want to run certain parts such as Nav2 on the dev machine during testing).
* (TO DO: add option to install full ROS2 env in the Elsabot Docker)
* Run RVIZ on that machine (or Foxglove which is better in most cases)
...
#### Open Foxglove in the browser on your dev host computer

* Load the provided JSON config file for Foxglove (foxglove/elsabot_robot_basic.json)
* Use Foxglove to set initial position on the map and then issue nav goals

#### Start RVIZ on development host  

* Load the rviz/navigation.rviz config file from this project.
* Use RVIZ to set initial position on the map and then issue nav goals
...
```  
ros2 launch elsabot_robot slam.launch.py
```
Move robot around using telop to build the map, and then save the map using:
```
ros2 run nav2_map_server map_saver_cli -f <path_to/elsabot_robot/maps/your_map_name> --ros-args -p save_map_timeout:=10000.
```
...
```
ros2 launch elsabot_robot bringup.launch.py use_gps:=True use_rtk:=False
```
...
### Start navigation using shell 2:
```
ros2 launch elsabot_robot navigation.launch.py use_gps:=True
```
