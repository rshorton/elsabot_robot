...
def generate_launch_description():
    ...
    elsabot_robot_dir = get_package_share_directory('elsabot_robot')

    gazebo_launch_path = PathJoinSubstitution(
        [FindPackageShare('elsabot_robot'), 'launch', 'gazebo.launch.py']
    )

    sensors_launch_path = PathJoinSubstitution(
        [FindPackageShare('elsabot_robot'), 'launch', 'sensors.launch.py']
    )

    joy_launch_path = PathJoinSubstitution(
        [FindPackageShare('elsabot_robot'), 'launch', 'joy_teleop.launch.py']
    )
    ...
    rviz_config_path = PathJoinSubstitution(
        [FindPackageShare('elsabot_robot'), 'rviz', 'navigation.rviz']
    )

    default_map_path = PathJoinSubstitution(
        [FindPackageShare('elsabot_robot'), 'maps', f'{MAP_NAME}.yaml']
    )

    default_keep_out_map_path = PathJoinSubstitution(
        [FindPackageShare('elsabot_robot'), 'maps', f'{MAP_NAME}_keep_out.yaml']
    )

    # Load the cmd_vel_mux config since it expects the parameters vs the
    cmd_vel_mux_config_file = os.path.join(get_package_share_directory('elsabot_robot'), 'config', 'cmd_vel_mux.yaml')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            name='map',
            default_value=os.path.join(elsabot_robot_dir, 'maps', f'{MAP_NAME}.yaml'),
            description='Full path to map yaml file to load'
        ),

        DeclareLaunchArgument(
            name='nav2_params_file',
            default_value=os.path.join(elsabot_robot_dir, 'config', 'navigation.yaml'),
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('elsabot_robot'), 'launch', 'sensor_fusion.launch.py')),
            launch_arguments={
                'use_sim_time': LaunchConfiguration('use_gazebo'),
                'use_gps': LaunchConfiguration('use_gps'),
            }.items()
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(get_package_share_directory('elsabot_robot'), 'launch', 'gps.launch.py')),
