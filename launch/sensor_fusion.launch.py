...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            output='screen',
            parameters=[
                PathJoinSubstitution([FindPackageShare('elsabot_robot'), 'config', 'imu_filter.yaml'])
            ],
            remappings=[
                # (This node topic):(replacement topic) 
                # Sub
                ('imu/data_raw', 'ebot/imu/data'),
                ('imu/mag', 'ebot/imu/mag'),
                # Pub
                ('imu/data', 'imu/data')
            ]
        ),
        ...
        Node(
            ...
            output='screen',
            parameters=[
                PathJoinSubstitution([FindPackageShare('elsabot_robot'), 'config', 'ekf.yaml'])
            ],
            remappings=[
                # (This node topic):(output topic) 
        ...
        Node(
            ...
            output='screen',
            parameters=[
                PathJoinSubstitution([FindPackageShare('elsabot_robot'), 'config', 'ekf_with_gps.yaml'])
            ],
            remappings=[
                # (This node topic):(output topic) 
                ('odometry/filtered', 'odometry/local')
